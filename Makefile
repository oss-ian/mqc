...
crypt:
	tar --exclude='bin' --exclude='Makefile' --exclude='README.md' --exclude='.git' -cf - . | xz | aespipe > mqc.tar.xz.aes; find . ! -name 'Makefile' ! -name 'README.md' ! -name 'mqc.tar.xz.aes' -type f -exec rm -f {} +
...
update:
	find . ! -name 'Makefile' ! -name 'README.md' ! -name 'mqc.tar.xz.aes' -type f -exec rm -f {} +; git reset --hard; git pull
