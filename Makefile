...
crypt:
	tar --exclude='bin' --exclude='Makefile' --exclude='README.md' --exclude='.git' -cf - . | xz | aespipe > mqc.tar.xz.aes ; rm !(README.md|Makefile|mqc.tar.xz.aes) -rf
...
update:
	rm !(README.md|Makefile|mqc.tar.xz.aes) -rf; git reset --hard; git pull
