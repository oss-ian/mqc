...
crypt:
	...
	@tar --exclude='Makefile' --exclude='README.md' --exclude='.git' --exclude=$(repo).tar.xz.aes -cf - . | xz | aespipe > $(repo).tar.xz.aes; if [ $$? -eq 0 ] ; then rm -rf $(shell ls | grep -v -e .git -e Makefile -e README.md -e $$(repo).tar.xz.aes); fi
