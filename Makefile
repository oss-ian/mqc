...
crypt:
	tar --exclude='bin' --exclude='Makefile' --exclude='README.md' --exclude='.git' -cvf - . | xz | aespipe > kek.tar.xz.aes ; rm -rf *.c *.h bin

decrypt:
	cat kek.tar.xz.aes | aespipe -d | xz -d | tar xvf -; rm kek.tar.xz.aes
