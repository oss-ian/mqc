...
crypt:
	tar --exclude='bin' --exclude='Makefile' --exclude='README.md' --exclude='.git' -cf - . | xz | aespipe > `basename pwd`.tar.xz.aes ; rm -rf *.c *.h bin

decrypt:
	cat `basename pwd`.tar.xz.aes | aespipe -d | xz -d | tar xf -; rm `basename pwd`.tar.xz.aes

push:
	git add `basename pwd`.tar.xz.aes; git commit -m `sha256sum `basename pwd`.tar.xz.aes`; git push -u origin master
